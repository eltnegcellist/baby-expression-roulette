...
const fileInput=$("fileInput"), frameCountEl=$("frameCount"), plainModeEl=$("plainMode"), previewSection=$("previewSection"), progressWrap=$("progressWrap"), progressBar=$("progressBar"), progressText=$("progressText"), grid=$("grid"), previewTitle=$("previewTitle"), previewNote=$("previewNote"), playBtn=$("playBtn"), reextractBtn=$("reextractBtn"), library=$("library");
...
let objectUrl=null, lastFile=null, selectedFrames=[], selectedFortunes=[], currentCreation=null, activeCreation=null, running=false, timer=null, currentIndex=0, primed=false;
...
function buildFortunes(count){
  const forts=BASE_FORTUNES.slice();
  let i=0;
  while(forts.length<count){ forts.push(EXTRA_FORTUNE_SEQUENCE[i % EXTRA_FORTUNE_SEQUENCE.length]); i++; }
  return shuffle(forts.slice(0,count));
}
function manualTargetCount(){const v=frameCountEl.value; return v==='auto'?null:Number(v);}
function autoTargetCount(duration,distinctCount,totalCount){
  const choices=[12,14,16,18,20,24,28];
  let desired=18;
  if(duration<4) desired=12;
  else if(duration<7) desired=14;
  else if(duration<12) desired=16;
  else if(duration<20) desired=18;
  else if(duration<35) desired=20;
  else if(duration<60) desired=24;
  else desired=28;
  const diversity=totalCount>0?distinctCount/totalCount:0.5;
  if(diversity<0.22) desired-=4;
  else if(diversity<0.35) desired-=2;
  else if(diversity>0.68) desired+=2;
  desired=Math.max(12,Math.min(28,desired));
  return choices.reduce((best,x)=>Math.abs(x-desired)<Math.abs(best-desired)?x:best,choices[0]);
}
function updatePreviewMeta(){
  const mode=modeName();
  previewTitle.textContent=mode==='omikuji' ? '自動で作成したおみくじ' : '自動で作成した表情ルーレット';
  previewNote.textContent=mode==='omikuji' ? '保存名は自動で付けます。必要なら保存後に変更できます。全運勢を最低1枚ずつ含みます。' : '保存名は自動で付けます。必要なら保存後に変更できます。';
}
plainModeEl.addEventListener('change',()=>{if(selectedFrames.length){if(modeName()==='omikuji')selectedFortunes=buildFortunes(selectedFrames.length);renderGrid();} updatePreviewMeta();});
...
async function loadVideo(file){ if(objectUrl) URL.revokeObjectURL(objectUrl); objectUrl=URL.createObjectURL(file); video.src=objectUrl; video.load(); if(video.readyState<1) await once(video,'loadedmetadata',10000); if(!isFinite(video.duration)||video.duration<=0) throw new Error('動画の長さを取得できません'); }
...
async function extractFromCurrentFile(){
  ...
  try{
    ...
    const candidates=[];
    for(let i=0;i<times.length;i++){ progressText.textContent=`候補を確認中... ${i+1}/${times.length}`; progressBar.style.width=(5+75*i/times.length)+'%'; candidates.push(await captureCandidate(times[i])); }
    ...
    selectedFortunes=buildFortunes(selectedFrames.length);
...
function renderGrid(){
  ...
  selectedFrames.forEach((f,i)=>{
    ...
    const fortune=selectedFortunes[i];
    d.innerHTML=`<img src="${f.dataUrl}"><div class="thumbInfo"><span>${(i+1)} / ${f.time.toFixed(1)}s</span>${showFortune?`<span class="fchip">${FORTUNE_ICONS[fortune]||'🎴'} ${fortune}</span>`:''}</div>`;
    grid.appendChild(d);
  });
}
...
function makeCreation(){
  const mode=modeName();
  const id=currentCreation?.id || ('r'+Date.now()+Math.random().toString(36).slice(2,8));
  return {
    id,
    title: currentCreation?.title || autoTitle(mode),
    createdAt: currentCreation?.createdAt || Date.now(),
    updatedAt: Date.now(),
    mode,
    frames: selectedFrames.map(f=>f.dataUrl),
    fortunes: mode==='omikuji' ? selectedFortunes.slice() : [],
    times: selectedFrames.map(f=>f.time)
  };
}
...
async function refreshLibrary(){
  ...
  all.forEach(x=>{
    ...
    el.innerHTML=`<img src="${x.frames[0]}"><div class="savedMain"><div class="savedTitle"></div><div class="savedMeta">${x.mode==='omikuji'?'おみくじ':'通常ルーレット'} ・ ${new Date(x.updatedAt||x.createdAt).toLocaleString('ja-JP')} ・ ${x.frames.length}枚</div><div class="savedBtns"><button class="primary open">遊ぶ</button><button class="secondary renameToggle">名前変更</button><button class="danger del">削除</button></div><div class="renameRow" style="display:none"><input type="text" value=""><button class="secondary saveName">保存</button></div></div>`;
    el.querySelector('.savedTitle').textContent=x.title;
    el.querySelector('.renameRow input').value=x.title;
    el.querySelector('.open').onclick=()=>preparePlay(x);
    el.querySelector('.renameToggle').onclick=()=>{ const row=el.querySelector('.renameRow'); row.style.display=row.style.display==='none'?'flex':'none'; };
    el.querySelector('.saveName').onclick=async()=>{ const nx=(el.querySelector('.renameRow input').value||'').trim(); if(!nx) return; x.title=nx; x.updatedAt=Date.now(); await dbPut(x); refreshLibrary(); };
...
function preparePlay(x){
  activeCreation=x; primed=false; running=false; clearInterval(timer); currentIndex=0;
  playSection.style.display='block'; startTitle.textContent=x.mode==='omikuji'?'赤ちゃんおみくじ':'表情ルーレット'; startOverlay.classList.add('show');
  fortuneBadge.style.display='none'; resultCard.style.display='none'; rouletteBadge.style.display='block'; rouletteBadge.textContent='タップでストップ'; tapHint.textContent='画像をタップすると止まります'; playImage.src=x.frames[0]; playSection.scrollIntoView({behavior:'smooth'});
}
let audioCtx=null;
...
function tone(freq,dur,when=0,vol=.09,type='triangle'){
  ...
  gain.gain.exponentialRampToValueAtTime(vol,now+.015);gain.gain.exponentialRampToValueAtTime(.0001,now+dur);
...
function playSound(name){
  ...
  if(name==='大吉'){tone(523,.95,1.05,.07,'sine');tone(659,.95,1.05,.07,'sine');tone(784,.95,1.05,.07,'sine');}
}
...
frameCountEl.addEventListener('change',()=>{if(lastFile){currentCreation=null;extractFromCurrentFile();}});
playBtn.addEventListener('click',async()=>{ if(!selectedFrames.length) return; await saveCurrentCreation(); preparePlay(currentCreation); });
primeBtn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();ensureAudio();primed=true;playSound('start');startOverlay.classList.remove('show');startRun(false);});
...
$('installHelp').addEventListener('click',()=>alert('このページをブラウザのメニューから「ホーム画面に追加」または「アプリをインストール」で使うと、アプリのように開けます。'));
