...
function runReplayRegressionTests(){
  const tests=[
    {name:'冒頭0.8秒→逆再生',pass:replayPlan(.8,true).route==='reverse-source'},
    {name:'中間12秒→元動画スロー',pass:replayPlan(12,true).route==='forward-source'},
    {name:'中間12秒・元動画なし→写真のみ',pass:replayPlan(12,false).route==='photo-only'},
    {name:'大吉→特大吉',pass:specialFortuneDisplay('miracle','大吉').fortune==='特大吉'},
    {
      name:'大凶→大逆転大吉',
      pass:(()=>{
...
miracleOverlay.innerHTML=
  ...
  '<div class="labMiracleActions">'+
    ...
    '<button id="labMiracleShare" type="button">↗ 共有</button>'+
...
const miracleShare=miracleOverlay.querySelector('#labMiracleShare');
...
miracleShare.addEventListener('click',async e=>{
  e.preventDefault();
  e.stopPropagation();
  clearTimeout(miracleAutoCloseTimer);
  miracleAutoCloseTimer=null;
